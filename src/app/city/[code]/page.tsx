...
export default function CityPage() {
  ...
  useEffect(() => {
    ...
    const cityCode = Array.isArray(params.code)
      ? params.code[0]
      : params.code || "";
  ...
  return (
    <div className="container mx-auto p-4 max-w-4xl">
      ...
      <div className="mt-6 w-full p-6 bg-white rounded-lg shadow-md">
        <h3 className="text-lg font-semibold mb-4 text-gray-800">
          지역별 상세 정보
        </h3>
        <p className="text-gray-600">
          {regionData.fullName}의 상세한 스마트팜 정보가 제공될 예정입니다.
        </p>
        <div className="mt-4   w-full ">
          <div className="p-4  w-full">
            <h4 className="font-medium">데이터 분석</h4>
            <p className="text-gray-600">
              아래 데이터 시각화 한 그래프가 들어갈 예정
            </p>
          </div>
        </div>
      </div>
